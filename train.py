...
parser = argparse.ArgumentParser(description="Training CMTL with multi-gpus")
parser.add_argument('--gpu_id', dest='gpu', default="1", type=str, help="Specify gpu id")
args = parser.parse_args()

method = 'cmtl' #method name - used for saving model file
dataset_name = 'shtechB' #dataset name - used for saving model file
output_dir = './saved_models_0828/' #model files are saved here

#train and validation paths
train_path = './data-peng/formatted_trainval/shanghaitech_part_B_patches_9/train'
train_gt_path = './data-peng/formatted_trainval/shanghaitech_part_B_patches_9/train_den'
val_path = './data-peng/formatted_trainval/shanghaitech_part_B_patches_9/val'
val_gt_path = './data-peng/formatted_trainval/shanghaitech_part_B_patches_9/val_den'
...
cuda_default = torch.device('cuda:{}'.format(args.gpu_id))

#load net and initialize it
net = CrowdCounter(ce_weights=class_wts, cuda=cuda_default)
network.weights_normal_init(net, dev=0.01)
net.to(device=cuda_default)
net.train()
...
with torch.cuda.device(args.gpu_id):
    for epoch in range(start_step, end_step+1):    
        ...
        for blob in data_loader:                
            ...
            density_map = net(im_data, gt_data, gt_class_label, class_wts, cuda=cuda_default)
            loss = net.loss
